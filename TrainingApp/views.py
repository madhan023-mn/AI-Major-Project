...
def LoadModel(request):
    if request.method == 'GET':
        ...
        context= {'data':"Rouge Score GPT-2 = 0.99"}
...
def SignupAction(request):
    if request.method == 'POST':
        ...
        status = "none"
        con = pymysql.connect(host='127.0.0.1',port = 3306,user = 'taskssqls', password = '@madhan23', database = 'AITraining',charset='utf8')
        with con:    
            cur = con.cursor()
            cur.execute("select * FROM register")
            rows = cur.fetchall()
            for row in rows:
                if row[0] == username:
                    ...
                    break
        if status == "none":
            db_connection = pymysql.connect(host='127.0.0.1',port = 3306,user = 'taskssqls', password = '@madhan23', database = 'AITraining',charset='utf8')
